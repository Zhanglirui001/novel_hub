...
export async function recovery<T>(path: string, init?: RequestInit): Promise<T> {
  const res = await fetch(`${await getApiBase()}/recovery${path}`, { ...init, headers: { ...(await apiHeaders()), ...init?.headers } });
  if (!res.ok) {
...
export async function backupText(name: string) {
  const res = await fetch(`${await getApiBase()}/recovery/backups/${encodeURIComponent(name)}`, { headers: await apiHeaders() });
...
export async function exportProjectFile(projectId: number) {
  const res = await fetch(`${await getApiBase()}/recovery/projects/${projectId}/export`, { headers: await apiHeaders() });
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
