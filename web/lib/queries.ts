...
export function useDailyCheckin(projectId: number) {
  return useQuery({
    queryKey: ["daily-checkin", projectId],
    queryFn: () => api.getDailyCheckin(projectId),
    enabled: Number.isFinite(projectId),
  });
}
...
function useDailyCheckinCache(projectId: number) {
  ...
  return (summary: Awaited<ReturnType<typeof api.getDailyCheckin>>) =>
    queryClient.setQueryData(["daily-checkin", projectId], summary);
}
